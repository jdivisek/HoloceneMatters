plotq.gbm <- function (x, i.var = 1, n.trees = x$n.trees, continuous.resolution = 51, 
          return.grid = FALSE, type = "link", ...) 
{
  ...
  for (i in 1:length(i.var)) {
    if (is.numeric(x$var.levels[[i.var[i]]])) {
      # grid.levels[[i]] <- seq(min(x$var.levels[[i.var[i]]]), max(x$var.levels[[i.var[i]]]), length = continuous.resolution)
      grid.levels[[i]] <- quantile(x$gbm.call$dataframe[,x$var.names[i.var[i]]], probs = seq(0,1, length.out = continuous.resolution))
    }
    else {
